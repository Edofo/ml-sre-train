...
.DEFAULT_GOAL := help
.PHONY: help test image cluster-up load deploy up monitoring-up monitoring-down slo-generate argocd-up gitops-bootstrap port-forward down tf-validate fmt
...
argocd-up: ## Install ArgoCD via Helmfile
	helmfile -f gitops/helmfile.yaml sync

gitops-bootstrap: ## Apply the root App-of-Apps
